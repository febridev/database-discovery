...
def get_instance_list():
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN mi.database_type = 'postgresql' THEN 'postgres'
...
def get_local_usernames(seq_id):
    ...
    # Menggunakan dictionary dengan username sebagai key untuk memudahkan perbandingan
    return {row[0]: UserInfo(*row) for row in local_usernames}
...
def main():
    ...
    for instance in instances:
        ...
        # Mengubah struktur data remote menjadi dictionary
        remote_usernames = {}
        original_remote_users = get_usernames(remote_conn, database_type)
        for user in original_remote_users:
            expired_date = user.expired_date.strftime('%Y-%m-%d') if isinstance(user.expired_date, datetime.date) else user.expired_date
            remote_usernames[user.username] = UserInfo(
        ...
        # Membandingkan berdasarkan username saja
        remote_keys = set(remote_usernames.keys())
        local_keys = set(local_usernames.keys())

        usernames_to_insert = remote_keys - local_keys
        usernames_to_delete = local_keys - remote_keys
        usernames_to_update = remote_keys & local_keys

        # Debug untuk set operations
        log_format("info", "=== DEBUG SET OPERATIONS ===")
        log_format("info", f"Remote keys: {remote_keys}")
        log_format("info", f"Local keys: {local_keys}")
        log_format("info", f"Usernames to insert: {usernames_to_insert}")
        log_format("info", f"Usernames to delete: {usernames_to_delete}")
        log_format("info", f"Usernames to update: {usernames_to_update}")

        # Debug untuk melihat data lengkap yang akan diproses
        log_format("info", "\n=== DEBUG DETAILED DATA ===")
        log_format("info", "Data to insert:")
        for username in usernames_to_insert:
            log_format("info", f"  {username}: {remote_usernames[username]}")

        log_format("info", "Data to delete:")
        for username in usernames_to_delete:
            log_format("info", f"  {username}: {local_usernames[username]}")

        log_format("info", "Data to update:")
        for username in usernames_to_update:
            log_format("info", f"  {username}:")
            log_format("info", f"    Remote: {remote_usernames[username]}")
            log_format("info", f"    Local: {local_usernames[username]}")

        # Proses insert
        for username in usernames_to_insert:
            user_data = remote_usernames[username]
            insert_usernames_header(
                seq_id,
                instance_name,
                user_data.username,
                user_data.expired_date,
                user_data.expired_in,
                user_data.status_user
            )

        # Proses delete
        for username in usernames_to_delete:
            user_data = local_usernames[username]
            mark_username_as_deleted(
                seq_id,
                instance_name,
                username,
                expired_date=user_data.expired_date,
                expired_in=user_data.expired_in
            )

        # Proses update
        for username in usernames_to_update:
            remote_data = remote_usernames[username]
            update_username_details(
                seq_id,
                instance_name,
                username,
                remote_data.expired_date,
                remote_data.expired_in,
                remote_data.status_user
            )
